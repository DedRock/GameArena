...
public class GameServiceImpl extends RemoteServiceServlet implements GameService {
    ...
    public boolean closeGame(int gameId, String myAccount) {
        ...
        try {
            ...
            resultSet = DbConnectorHolder.getInstance().executeQuery("SELECT * FROM games WHERE id='" + gameId + "'");
            if (resultSet.next()){
                ...
                if (myAccount.equals(player1) && (player2.isEmpty())){
                    DbConnectorHolder.getInstance().execute("DELETE games FROM games WHERE id=" + gameId);
                    return true;
                }
                ...
                if (!player1.isEmpty() && !player2.isEmpty() && !gameEnd ){
                    ...
                    DbConnectorHolder.getInstance().execute("UPDATE games SET gameEnd=TRUE, gameTerminated=TRUE, winPlayer='" + opponent + "' WHERE id=" + gameId);
    ...
    public int createGame(String myAccount) {
        ...
        try {
            ...
            DbConnectorHolder.getInstance().execute("INSERT games (player_1, player_2, gameEnd, winPlayer, gameType) VALUES ('" + myAccount + "', '', false, '', 'XO')");
            resultSet = DbConnectorHolder.getInstance().executeQuery("SELECT id FROM games WHERE player_1 = '" + myAccount + "' AND player_2 = '' AND gameEnd = false AND gameType='XO'");
            if ( resultSet.next() )
    ...
    public List<SimpleGameInfo> getListOfOpenGames(String myAccount) {
        ...
        try {
            ...
            resultSet = DbConnectorHolder.getInstance().executeQuery("SELECT id, player_1, player_2, gameType FROM games WHERE gameEnd=false");
    ...
    public boolean joinToOpenGame(int gameId, String myAccount) {
        try {
            ...
            resultSet = DbConnectorHolder.getInstance().executeQuery("SELECT player_2 FROM games WHERE id=" + gameId);
            if (resultSet.next())
                // Если место вакантно
                if (resultSet.getString("player_2").isEmpty()){
                    // Добавим в таблицу games к записи с "id" = gameId себя как "player_2"
                    DbConnectorHolder.getInstance().execute("UPDATE games SET player_2='" + myAccount + "' WHERE id=" + gameId);
                    DbConnectorHolder.getInstance().commit();
                    return true;
                }
        } catch (SQLException e) {
    ...
    public GameXO_State updateGameState(int gameId) {
        ...
        try {
            ...
            resultSet = DbConnectorHolder.getInstance().executeQuery("SELECT gameTerminated FROM games WHERE id=" + gameId);
            if (resultSet.next()){
            ...
            resultSet = DbConnectorHolder.getInstance().executeQuery("SELECT * FROM gamestate WHERE id = '" + gameId + "'");
    ...
    public String waitForOpponent(int gameId) {
        ...
        try {
            resultSet = DbConnectorHolder.getInstance().executeQuery("SELECT player_1, player_2 FROM games WHERE id=" + gameId);
            if (resultSet.next()){
                if (!resultSet.getString("player_2").isEmpty()){
                    String player_2 = resultSet.getString("player_2");
                    DbConnectorHolder.getInstance().execute("INSERT gamestate (id, curPlayer) VALUES ('" + gameId + "', '" + resultSet.getString("player_1") + "')");
                    opponent = player_2;
                }
            }
        } catch (SQLException e) {
            ...
            if (resultSet.next()){
                ...
                if (myAccount.equals(player1) || myAccount.equals(player2)){
                    ...
                    if (!result.getWin()){
                        ...
                        DbConnectorHolder.getInstance().execute("UPDATE gamestate SET curPlayer='" + curPlayer +
                        ...
                        DbConnectorHolder.getInstance().execute("UPDATE games SET gameEnd=1, winPlayer='" + result.getWinPlayer() + "'");
                        DbConnectorHolder.getInstance().commit();
                    }
                }
            }
